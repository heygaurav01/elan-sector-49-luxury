      ...
      <div className="container mx-auto px-4 py-2 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-4">
          {NAV_ITEMS.map((item) => (
            <a
              key={item.name}
              href={item.href}
              className="text-sm font-medium text-gray-800 hover:text-primary transition-colors uppercase tracking-wide"
            >
              {item.name}
            </a>
          ))}
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          <Button
            asChild
            size="sm"
            className="bg-[#25D366] hover:bg-[#20BD5A] text-white flex items-center justify-center gap-2 h-8 w-8 p-0 lg:h-auto lg:w-auto lg:px-4 lg:py-2"
            ...
            <nav className="flex flex-col gap-4 mt-8">
              {NAV_ITEMS.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  className="text-lg font-medium text-gray-800 hover:text-primary transition-colors uppercase tracking-wide"
